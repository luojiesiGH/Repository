...
class BS4Parser(object):
    ...
    def craw(self):
        ...
        curlsT = threading.Thread(target=self.collecturls,name="CollectUrl",args=(1,))
        curlsT.start()
        for i in range(10):
            self.spiderpools.append(threading.Thread(target=self.collectpageinfo))
        self.spiderpools.append(threading.Thread(target=self.outputer.output_xls))
        ...
        while True:
            ...
            while not self.urlq.empty():
                ...
                try:
                    ...
                    try:
                        data["postion"] = CompanyPage.find("div", "tHeader tHjob").find("h1")["title"]
                        data["lname"] = CompanyPage.find("div", "tHeader tHjob").find("span", "lname").string
                        data["strong"] = CompanyPage.find("div", "tHeader tHjob").find("strong").string
                        data["cname"] = CompanyPage.find("div", "tHeader tHjob").find("p", "cname").find("a")["title"]
